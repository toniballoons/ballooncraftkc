...
// Only apply CSS vars when actually saving — not on hover
...
function ThemeCard({ theme, isActive, onApply }) {
  return (
    ...
    >
      {/* Preview swatch — uses inline styles so no CSS var changes needed */}
      <div
        className="h-28 w-full relative flex items-end"
        style={{
          background: `linear-gradient(135deg, ${theme.preview[0]}, ${theme.preview[1] || theme.preview[0]}, ${theme.preview[2] || theme.preview[0]})`,
        }}
      >
        {/* Simulated nav bar */}
        <div
          className="absolute top-0 left-0 right-0 h-7 flex items-center px-2 gap-1"
          style={{ background: theme.nav.bg || 'rgba(0,0,0,0.3)' }}
        >
          <span className="text-[9px] font-bold" style={{ color: theme.nav.logoColor || '#fff' }}>
            ● {theme.name}
          </span>
      ...
      <div className="p-2.5 bg-white">
        ...
        <p className="text-[10px] text-muted-foreground leading-tight line-clamp-2 mb-1.5">
          {theme.description}
        </p>
...
export default function ThemeSettings() {
  ...
  const handleApply = (id) => {
    ...
    // Apply CSS vars only when saving — no hover flicker
    applyThemeCss(t);
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Choose from {THEMES.length} designs. Click <strong>Apply</strong> on any theme to save it to your live site.
          </p>
        </div>

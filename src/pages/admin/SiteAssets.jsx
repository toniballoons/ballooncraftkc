    ...
    <div className="rounded-xl overflow-hidden border bg-card shadow-sm group">
      <div className="aspect-video bg-muted relative">
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
          <label className="cursor-pointer bg-white text-black text-xs font-semibold px-3 py-2 rounded-lg flex items-center gap-1.5 hover:bg-gray-100">
            <Upload className="w-3.5 h-3.5" /> {uploading ? 'Uploading...' : 'Replace'}
            <input type="file" accept="image/*" className="hidden" onChange={handleFile} disabled={uploading} ref={fileRef} />
          </label>
    ...
    <div className="space-y-8 max-w-6xl">
      ...
      {allImagesByPage.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <Link to="/admin/pages"><Button className="mt-4" size="sm">Go to Page Editor</Button></Link>
        </div>
      ) : (
